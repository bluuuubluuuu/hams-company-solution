...
| 1.6a | 2026-07-24 | **Device-verified on `ALI-NX1`:** DV1 — a Wi-Fi-on release delivers all cuts and emits a clean `304` (no `lost_*`); DV2 — a gateway-miss release strands the cuts (`pushed = 2`) and holds a local `302 lost_cuts=N` that never reaches Wialon. Reframed `302` as a **local diagnostic, best-effort push**: it most often fires *because* Wialon was unreachable, so it usually cannot land — do not rely on Wialon to surface stranded work; that is a future n8n-side count. |
