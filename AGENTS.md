...
# AGENTS.md - HAMS Task Recorder (V6)

> This file is the primary instruction set for Codex (terminal) and Codex CLI.
...
>
> **Note:** Codex does NOT automatically load `CONTEXT.md`. Read it explicitly at the start of any task that touches IPS, REST, coordinate conversion, or Wialon configuration.
...
|--|--|--|
| `yhwoo516@gmail.com` (Codex Pro) | WYH (dev) | Code generation, cross-checking, reading docs, integration testing |
...
- Produces: updated replacement files for AGENTS.md, CONTEXT.md, HAMS_EVENT_CODE_DICTIONARY.md, HAMS_API_TESTING.md
...
**Net impact for Codex and Codex:**
...
- Keep task-event outbound `event_code` values restricted to `179`, productive `180`, and `35`; diagnostics telemetry Option B is FINAL and additionally pushes **29/40/24/25/41/42/26/27/43/44** through the separate `diagnostics` / `TelemetryPushEngine` path (see `docs/HAMS_EVENT_CODE_DICTIONARY.md` v1.3)
- Preserve the socket transport, login frame, and harvest/heartbeat task frame path; diagnostics telemetry uses `telemetryFrame` separately and must not inflate task pending counts
...
- Tasks 2.1-2.7 - [done] COMPLETE and committed.
...
- Event-code policy v1.3 (`docs/HAMS_EVENT_CODE_DICTIONARY.md`): task frames push only `179`, productive `180`, and `35`; diagnostics telemetry Option B pushes final verified codes **29/40/24/25/41/42/26/27/43/44**; legacy `279/280/281/283/284/291/292/293` remain HAMS-local.
...
- **Task 2.7.5 - GPS streaming + armband UI + DB v2 + daily rollover** - [pending] IMPLEMENTED, **pending field verification 2026-05-07**. Bundles four related changes shipped together over 2026-05-05/06:
...
- Latitude: 2.268721 deg N, Longitude: 103.282985 deg E
...
> versioned. Current Room database version: **5** (see Schema migrations below).
...
Task events follow the local capture pipeline below. Diagnostics telemetry events use the `diagnostics` table and the final Option B telemetry code mapping.
event is a Wialon event. Event capture writes to SQLite synchronously; the push
boundary later selects task-event codes 179, productive 180, and 35; the telemetry push path separately drains diagnostics rows with final Option B codes.
...
| GPS lock transition | `GpsLockTransition` on `GpsLockState` changes | Records final diagnostics telemetry codes **24** gps_lost and **25** gps_recovery with dwell filtering. |
...
| Diagnostics telemetry Option B | Boot receiver, shutdown tracker, screen/power receivers, GPS lock transition, motion detector | Final device + Wialon verified codes: **29** boot, **40** shutdown, **24/25** GPS lost/recovery, **41/42** stop/start moving, **26/27** screen off/on, **43/44** power connected/disconnected. |

### Push eligibility rule

Task-event push eligibility is intentionally narrow (from `docs/HAMS_EVENT_CODE_DICTIONARY.md`):
...
- **HAMS-local legacy values (279/280/281/283/284/291/292/293)**: do not push unless Wialon-side reporting config is explicitly approved. This does not include the final diagnostics telemetry Option B codes.
...
as custom Wialon `event_code` values. Every pushed task frame and diagnostics telemetry frame carries
...
- **Push target**: tasks with `push_status='pending'` only. Active tasks are insulated - push never finalizes them. Lifecycle is worker-controlled (NEW TASK 3s hold) or Android-lifecycle-controlled (app swipe -> `auto_killed`, day rollover -> `auto_rollover`, periodic in-process rollover at 1 s tick -> `auto_rollover`). The legacy `repo.autoSaveActiveOnWifi(...)` pre-flight hook was **removed** 2026-08-05 (no callers remained).
...
- **Task query**: `EventDao.getPending` - `pushed = 0 AND event_code IN (179, 180, 35) AND NOT (event_code = 180 AND work_count <= 0)`, ordered by `timestamp ASC, id ASC`. Diagnostics telemetry is drained separately from `DiagnosticDao.pendingDiagnostics()` by `TelemetryPushEngine`.
...
The task push engine is mostly data-driven. `IPSFrameBuilder` derives `ffb_cut` from task `event_code` values, while diagnostics telemetry uses `TelemetryCode` + `telemetryFrame` and does not touch `PushController.pendingCountFlow`.

### Auto + Manual cooperation contract (Task 2.8 spec -section 17)
...
### Force-stop / battery-restriction recovery (Task 2.8 spec -section 18)
...
### Retention sweep (Codex adversarial-fix design -section 4, 2026-05-15)
...
- **event_code_new_task / auto_save / battery legacy keys**: local/internal identifiers only under the v1.3 policy. Final diagnostics telemetry codes are not config keys; they are locked in `TelemetryCode` as **29/40/24/25/41/42/26/27/43/44**.
...
Do not add or push any other custom Wialon `event_code` values without a matching Wialon report/sensor/notification design.
...
App stays open across MYT midnight -> periodic 1s tick re-runs rolloverActiveTaskIfStale (Task 2.8 spec -section 4.1)
...
GPS lock lost/recovered - diagnostics telemetry writes final Option B codes **24/25** after dwell filtering
```
...
**Codex handoff rule:** Codex reads this file (`AGENTS.md`) and `CONTEXT.md` as its primary instructions. Do not duplicate information between the two - AGENTS.md is the rule set, CONTEXT.md is the reference data.
...
|--|--|--|
| `AGENTS.md` | Android build rules (this file) | Building or modifying the app |
...
4. `AGENTS.md` (this file) - understand how to build
